...
class TestComprehensiveIntegrationComplete:
    """Comprehensive integration tests for complete text message flow, multi-user scenarios, 
    voice messages, and connection recovery - Requirements: 2.1, 2.2, 3.1, 3.2, 2.5"""
    ...
    @pytest.mark.asyncio
    async def test_message_ordering_and_consistency(self, services):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_room_operations(self, services, mock_websocket_factory):
    ...
    @pytest.mark.asyncio
    async def test_stress_testing_with_failures(self, services, mock_websocket_factory):
        """Test system behavior under stress with intermittent failures"""
        connection_manager, room_manager, message_handler = services
        room_id = "stress-test-room"
        
        # Create connections with mixed reliability
        stable_connections = []
        unstable_connections = []
        
        # Create 10 stable connections
        for i in range(10):
            ws = mock_websocket_factory()
            conn = await connection_manager.connect(ws, room_id, f"stable_user_{i}")
            stable_connections.append((ws, conn))
        
        # Create 5 unstable connections that will fail intermittently
        for i in range(5):
            ws = mock_websocket_factory()
            # Make send_text fail 50% of the time
            ...
            async def intermittent_failure(*args, **kwargs):
                ...
                return await original_send_text(*args, **kwargs)
            
            ws.send_text = AsyncMock(side_effect=intermittent_failure)
            conn = await connection_manager.connect(ws, room_id, f"unstable_user_{i}")
            unstable_connections.append((ws, conn))
        
        # Verify all connections established
        assert connection_manager.get_room_connection_count(room_id) >= 10  # Only stable connections are guaranteed to remain
        
        # Send messages from stable connections under stress
        stress_tasks = []
        for i, (ws, conn) in enumerate(stable_connections):
            ws.send_text.reset_mock()
            
            # Send multiple messages per connection
            for j in range(3):
                message = TextMessage(
                    id=f"stress_msg_{i}_{j}",
                    user_id=conn.user_id,
                    room_id=room_id,
                    content=f"Stress message {i}-{j}"
                )
                task = asyncio.create_task(
                    message_handler.handle_text_message(conn, message)
                )
        ...
        # Verify most messages succeeded despite unstable connections
        successful_results = [r for r in results if r is True]
        assert len(successful_results) >= 25  # At least 83% success rate (25 out of 30)
        ...
        assert room_stats["connection_count"] >= 10  # Stable connections should remain