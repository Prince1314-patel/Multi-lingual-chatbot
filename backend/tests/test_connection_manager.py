...
async def test_broadcast_to_room(connection_manager):
    ...
    await connection_manager.connect(ws3, "other_room", "user3")
    ...
    assert sent_count == 2
    assert len(ws1.messages_sent) == 2  # Join message from user2 + broadcast
    assert len(ws2.messages_sent) == 1  # Only the broadcast (user2 joined after user1)
...
def test_get_room_stats(connection_manager):
    ...
    stats = connection_manager.get_room_stats("non_existent")
...
async def test_get_total_connections_with_data(connection_manager):
    ...
    await connection_manager.connect(ws1, "room1", "user1")
    await connection_manager.connect(ws2, "room2", "user2")
